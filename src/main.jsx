...
function QuestionPage({ roleId }) {
  ...
  const query = new URLSearchParams(window.location.search);
    const organizerMode = roleId === 4;
  const puzzleVersion = query.get('puzzle') === 'backup' ? 'backup' : 'primary';
  const [question, setQuestion] = useState(null);
  const [selected, setSelected] = useState(null);
  const [confirmed, setConfirmed] = useState(false);
  /*
    return <Layout><main className={`question-page role-theme-${role.accent}`}><div className="question-head reveal"><a className="back-link" href={`/${team ? `?team=${encodeURIComponent(team)}` : ''}`}>← All stations</a><div className="question-label"><span>ROLE {String(roleId).padStart(2, '0')}</span><span>TEAM {team || '—'}</span></div><h1>{question.roleName}</h1><p className="mission">{question.description}</p></div><section className="challenge-panel reveal">{organizerMode && <div className="organizer-panel"><p className="rules-title">Need an alternative?</p><strong>No Kannada-familiar player?</strong><p>If nobody in your group is comfortable with Kannada cinema, use the alternative Bollywood puzzle.</p><div className="organizer-actions"><button className="button" onClick={chooseBackup}>No Kannadiga</button></div></div>}<div className="panel-kicker"><span>CHALLENGE / {question.title}</span>
  */
